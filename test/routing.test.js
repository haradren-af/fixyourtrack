...
test('uses distinct profile-aware BRouter requests for cycling and walking', () => {
  ...
  assert.equal(cycling.length, 4)
  assert.equal(walking.length, 2)
  assert.match(cycling[2].url, /profile=hiking-beta/)
  assert.match(cycling[3].url, /routed-foot/)
  assert.equal(cycling[1].minimumIntervalMs, 1000)
  assert.equal(cycling[3].minimumIntervalMs, 1000)
